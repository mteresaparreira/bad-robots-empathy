# superBAD_bilstm_k_fold_cv_overlapping.py
...
def executeModel_BiLSTM_with_k_fold_cv(df, nums_folds = 5, results_directory='./', seed_value = 42, sequence_length = 1, unit = 64, dropout = 0.2, activation = 'softmax', loss = 'categorical_crossentropy', optimizer = 'adam', epoch = 100, batch_size = 32):
    ...
    with open(f'{results_directory}/k_fold_cv_overlapping_data_results_BiLSTM_final.txt', 'a') as results_file:
        results_file.write("Final BiLSTM model run on 5 FOLD CV on Overlapping participant data" + "\n")
...
def main():
    ...
    results_directory = '../../results/' + 'k_fold_cv_overlapping_data_bilstm_final_' + now.strftime("%Y-%m-%d_%H-%M-%S") + '/'
