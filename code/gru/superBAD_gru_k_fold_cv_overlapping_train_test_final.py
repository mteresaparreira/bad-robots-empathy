# superBAD_gru_k_fold_cv_overlapping.py

"""
Description: This script trains a GRU Model with K-FOLD Cross Validation on Overlapping participant data where the facial data is extracted from the OpenFace library to classify human reactions into Control, Failure Human, Failure Robot classes
...
def executeModel_GRU_with_k_fold_cv(df, nums_folds = 5, results_directory='./', seed_value = 42, sequence_length = 1, unit = 64, dropout = 0.2, activation = 'softmax', loss = 'categorical_crossentropy', optimizer = 'adam', epoch = 100, batch_size = 32):

    """
    executeModel_GRU_with_k_fold_cv(): takes in the dataFrame along with the directory specification & hyper-parameters and trains a GRU model on the best performing hyper-parameter on a k-fold cross validation with overlapping participant data
    ...
    with open(f'{results_directory}/k_fold_cv_overlapping_data_results_GRU_final.txt', 'a') as results_file:
        results_file.write("Final GRU model run on 5 FOLD CV on Overlapping participant data" + "\n")
...
def main():
    ...
    results_directory = '../../results/' + 'k_fold_cv_overlapping_data_gru_3k_final_' + now.strftime("%Y-%m-%d_%H-%M-%S") + '/'
    ...
    units = 32 #[32, 64, 128]
    dropouts = 0.6 # [0, 0.2, 0.4, 0.6]
    activations = 'relu' #['relu', 'tanh', 'softmax']
    losses = 'hinge' #['categorical_crossentropy'# , 'sparse_categorical_crossentropy', 'binary_crossentropy', 'hinge']
    optimizers = 'SGD' #['SGD', 'Adam']
    epochs = 3000 #[250, 500, 1000]
    ...
    executeModel_GRU_with_k_fold_cv(
        superBAD_df,
        nums_folds,
        results_directory,
        seed_value = 42,
