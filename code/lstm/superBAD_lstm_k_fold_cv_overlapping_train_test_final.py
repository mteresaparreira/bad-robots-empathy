# superBAD_lstm_k_fold_cv_overlapping.py

"""
Description: This script trains a LSTM Model with K-FOLD Cross Validation on Overlapping participant data where the facial data is extracted from the OpenFace library to classify human reactions into Control, Failure Human, Failure Robot classes
...
def executeModel_LSTM_with_k_fold_cv(df, nums_folds = 5, results_directory='./', seed_value = 42, sequence_length = 1, unit = 64, dropout = 0.2, activation = 'softmax', loss = 'categorical_crossentropy', optimizer = 'adam', epoch = 100, batch_size = 32):

    """
    executeModel_LSTM_with_k_fold_cv(): takes in the dataFrame along with the directory specification & hyper-parameters and trains a LSTM model on the best performing hyper-parameter on a k-fold cross validation with overlapping participant data
    ...
    with open(f'{results_directory}/k_fold_cv_overlapping_data_results_LSTM_final.txt', 'a') as results_file:
        results_file.write("Final LSTM model run on 5 FOLD CV on Overlapping participant data" + "\n")
    for fold_indx in range(nums_folds):
        try:
            ...
            # Retrieve y_train, y_test: values corresponding to same indexes, from labels_ohe
            ...
            with tf.device('/GPU:0'):  # Use GPU 0
                ...
                with open(f'{results_directory}/k_fold_cv_overlapping_data_results_LSTM_final.txt', 'a') as results_file:
                    results_file.write("\n")
                    results_file.write("---------FINAL TEST RESULTS---------" + "\n")
...
def main():
    ...
    results_directory = '../../results/' + 'k_fold_cv_overlapping_data_lstm_final_' + now.strftime("%Y-%m-%d_%H-%M-%S") + '/'
    ...
    units = 128 #[32, 64, 128]
    dropouts = 0.2 # [0, 0.2, 0.4, 0.6]
    activations = 'relu' #['relu', 'tanh', 'softmax']
    losses = 'binary_crossentropy' #['categorical_crossentropy'# , 'sparse_categorical_crossentropy', 'binary_crossentropy', 'hinge']
    optimizers = 'SGD' #['SGD', 'Adam']
    epochs = 7000 #[250, 500, 1000]
    batch_sizes = 1024 # [512, 1024, 2048, 4096]
    ...
    executeModel_LSTM_with_k_fold_cv(
        superBAD_df,
        nums_folds,
        results_directory,
        seed_value = 42,
