...
Model: DecisionTrees with manual 5-fold cross-validation to create folds having non-overlapping participant data.
...
def trainModel(superBAD_df, results_directory='.', seedValue = 42):
    ...
    with open(f'{results_directory}superBAD_k_fold_cv_decisionTrees.txt', 'a') as output_file:
        output_file.write(f"Model Selection: classifier = DecisionTreeClassifier(ccp_alpha=0.0, class_weight=None, criterion='entropy', max_depth=None, max_features='auto', max_leaf_nodes=None, \
    ...
    for fold_indx in tqdm(range(len(train_folds)), desc='FOLD', total=len(train_folds)):
        ...
        confusion_matrix_path = results_directory + f'superbad_decisionTrees_with_k_fold_cv_at_fold_{fold_indx + 1}.png'
        ...
        with open(f'{results_directory}superBAD_k_fold_cv_decisionTrees.txt', 'a') as output_file:
            output_file.write('---------------------------------------------------------------------' + '\n')
            output_file.write('Begin Model: DecisionTrees with K-FOLD Cross Validation for Non-overlapping participant data' + '\n')
...
def main():
    ...
    results_directory = '../../../results/' + 'k_fold_cv_decisionTrees_' + now.strftime("%Y-%m-%d_%H-%M-%S") + '/'
