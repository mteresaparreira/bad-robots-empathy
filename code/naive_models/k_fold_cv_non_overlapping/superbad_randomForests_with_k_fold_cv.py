...
Model: RandomForests with manual 5-fold cross-validation to create folds having non-overlapping participant data.
...
def trainModel(superBAD_df, results_directory='.', seedValue = 42):
    ...
    with open(f'{results_directory}superBAD_k_fold_cv_randomForests.txt', 'a') as output_file:
        output_file.write(f"Model Selection: classifier = RandomForestClassifier(criterion='entropy', max_features=None, n_estimators=500, random_state=42, verbose=True)")
    ...
    for fold_indx in tqdm(range(len(train_folds)), desc='FOLD', total=len(train_folds)):
        ...
        confusion_matrix_path = results_directory + f'superbad_randomForests_with_k_fold_cv_at_fold_{fold_indx + 1}.png'
        ...
        with open(f'{results_directory}superBAD_k_fold_cv_randomForests.txt', 'a') as output_file:
            output_file.write('---------------------------------------------------------------------' + '\n')
            output_file.write('Begin Model: RandomForests with K-FOLD Cross Validation for Non-overlapping participant data' + '\n')
...
def main():
    ...
    results_directory = '../../results/' + 'k_fold_cv_randomForests_' + now.strftime("%Y-%m-%d_%H-%M-%S") + '/'
