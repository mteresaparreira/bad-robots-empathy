...
def parseResultsFile(file_path):
    ...
    for search in search_combinations:
        ...
        for line in search:
            ...
            if re.search(r'CONFUSION MATRIX STORED AT PATH', line):
                confusion_matrix = line.split(r'CONFUSION MATRIX STORED AT PATH :')[-1]   # Extract file name

            class_report_start = re.search(r'---------CLASSIFICATION REPORT-----------', line)
            if class_report_start:
                # print(class_report_start)
                capture_classification = True  # Start capturing content
                continue  # Skip this line as it's just a marker
            elif capture_classification and re.search(r'------------------END------------------', line):
...
def main():
    ...
    file_path = '../../../model_results/naiveModels_results/k_fold_cv_overlapping_data_XGBoost_2023-09-28_03-30-26/superBAD_k_fold_cv_overlapping_data_XGBoost.txt'
    output_path = '../../../model_results/naiveModels_results/k_fold_cv_overlapping_data_XGBoost_2023-09-28_03-30-26/superBAD_k_fold_cv_overlapping_data_XGBoost.csv'
