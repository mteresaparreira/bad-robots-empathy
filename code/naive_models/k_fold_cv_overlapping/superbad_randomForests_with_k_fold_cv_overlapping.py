...
Model: RandomForests with 5-fold cross-validation to create folds having overlapping participant data.
...
def trainModel(superBAD_df, num_folds=5, results_directory='.', seedValue = 42):
    ...
    with open(f'{results_directory}superBAD_k_fold_cv_overlapping_data_randomForests.txt', 'a') as output_file:
        output_file.write(f"Model Selection: classifier = RandomForestClassifier(criterion='entropy', max_features=None, n_estimators=500, random_state=42, verbose=10)")
    ...
    for fold_indx in tqdm(range(len(train_folds)), desc='FOLD', total=len(train_folds)):
        ...
        classifier = RandomForestClassifier(criterion='entropy', max_features=None, n_estimators=500, random_state=42, verbose=10)
        ...
        confusion_matrix_path = results_directory + f'superbad_randomForests_with_k_fold_cv_overlapping_data_at_fold_{fold_indx + 1}.png'
        ...
        with open(f'{results_directory}superBAD_k_fold_cv_overlapping_data_randomForests.txt', 'a') as output_file:
            output_file.write('---------------------------------------------------------------------' + '\n')
            output_file.write('Begin Model: RandomForests with K-FOLD Cross Validation for Overlapping Participant Data' + '\n')
...
def main():
    ...
    results_directory = '../../../results/' + 'k_fold_cv_overlapping_data_randomForests_' + now.strftime("%Y-%m-%d_%H-%M-%S") + '/'
