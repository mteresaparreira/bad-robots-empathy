...
def trainModel(df, param_grid, results_directory):
    ...
    # LogisticRegression without GridSearch
    classifier = LogisticRegression(max_iter=500, multi_class = 'ovr', random_state = 42)
    ...
    confusion_matrix_path = results_directory + 'logisticRegression_confusion_matrix_beforeGridSearch.png'
    plt.savefig(confusion_matrix_path)
    plt.clf()

    with open(f'{results_directory}/superBAD_logisticRegression.txt', 'a') as output_file:
        output_file.write('---------------------------------------------------------------------')
        output_file.write('\n' + 'Begin Model: LogisticRegression without GridSearch' + '\n')
    ...
    grid_search = GridSearchCV(LogisticRegression(multi_class = 'ovr', random_state = 42), param_grid=param_grid, refit= True, verbose = 10, error_score = 999999999)
    ...
    df.to_csv(f'{results_directory}/logisticRegression_grid_search_cross_validation_results.csv')
    ...
    # Save the confusion matrix as an image
    confusion_matrix_path = results_directory + 'logisticRegression_confusion_matrix_afterGridSearch.png'
    plt.savefig(confusion_matrix_path)
    plt.clf()

    with open(f'{results_directory}/superBAD_logisticRegression.txt', 'a') as output_file:
        output_file.write('---------------------------------------------------------------------' + '\n')
        output_file.write('Begin Model: LogisticRegression with GridSearch' + '\n')
        output_file.write('Dataset: allParticipants_5fps, full_n' + '\n')
        output_file.write('---------------------------------------'+ '\n')
...
def main():
    ...
    results_directory = '../../results/' + 'LogisticRegression_' + now.strftime("%Y-%m-%d_%H-%M-%S") + '/'
    ...
    'penalty': ['l1', 'l2'],  # Use 'l1' or 'l2' penalties
    'solver': ['liblinear', 'saga']  # Use a solver that supports 'l1' penalty
    },{
    'C': [0.01, 0.1, 1, 10, 100],
    'max_iter': [100, 500, 1000],
    'penalty': ['l2'],  # Use only 'l2' penalty
    'solver': ['lbfgs', 'newton-cg', 'sag'],  # Use 'lbfgs', 'newton-cg', 'sag' solver
    }
]
