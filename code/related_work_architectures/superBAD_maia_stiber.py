...
def createDataSplits(df, results_directory= '.', seed_value = 42):
...
#         print(X_test.shape, y_test.shape)
        ...
        """
        return num_classes, X_train, X_val, X_test, y_train, y_val, y_test 

    except Exception as e:
        with open(f'{results_directory}/results_related_work.txt', 'a') as results_file:
            results_file.write(
...
def executeModel(df, results_directory, seed_value = 42, dropouts = 0.2, activations = 'softmax', losses = 'categorical_crossentropy', optimizers = 'adam', epochs = 100, batch_sizes = 32):

    """
    executeModel(): takes in the dataFrame along with the directory specification & hyper-parameters and trains a model
                    ...
                    for epoch in epochs:
                        for i, batch_size in enumerate(batch_sizes):
                            try:
                                ...
                                # # Retrieve the splits
                                num_classes, X_train, X_val, X_test, y_train, y_val, y_test = createDataSplits(df, results_directory, seed_value)
                                ...
                                with tf.device('/GPU:0'):  # Use GPU 0
                                    ...
                                    with open(model_data_path + f'model_{dropout}_{activation}_{loss}_{optimizer}_{epoch}_{batch_size}', 'wb') as f:
                                    ...
                                    confusion_matrix_path = results_directory + f'confusion_matrices/confusion_matrix_{dropout}_{activation}_{loss}_{optimizer}_{epoch}_{batch_size}.png'
                        ...
                        batch_size_accuracy_path = batch_size_results_path + f'batch_size_comparison_{dropout}_{activation}_{loss}_{optimizer}_{epoch}.png'
                        ...
                        with open(f'{results_directory}/results_related_work.txt', 'a') as results_file:
                            ...
                            results_file.write(f'BATCH_SIZE plots saved for batch_size_comparison_{dropout}_{activation}_{loss}_{optimizer}_{epoch}.png' + '\n')
                            results_file.write("------------ BATCH_SIZE PLOTS ------------" + "\n")
                        """
                        End: Logging 
                        ------------------------------------------------------------------------------------
...
def main():
    ...
    results_directory = '../../results/' + 'realted_work_arch_maia_stiber_' + now.strftime("%Y-%m-%d_%H-%M-%S") + '/'
