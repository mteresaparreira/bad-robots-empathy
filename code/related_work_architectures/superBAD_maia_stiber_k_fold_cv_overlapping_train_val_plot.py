...
def executeModel(df, nums_folds = 5, results_directory='./', seed_value = 42, dropout = 0.2, activation = 'softmax', loss = 'categorical_crossentropy', optimizer = 'adam', epoch = 100, batch_size = 32):
    ...
    with open(f'{results_directory}/k_fold_cv_overlapping_data_results_related_work_maia_stiber_arch.txt', 'a') as results_file:
        ...
        NOTE: This script makes use of the test data as the validation data - just to check model performance - through the train-validation plots\
        The same test_data is used for predictions - IGNORE THE FINAL TEST RESULTS FROM THIS SCRIPT\
    ...
    for fold_indx in range(nums_folds):
        try:
            ...
            NOTE: This script makes use of the test data as the validation data - just to check model performance - through the train-validation plots
            The same test_datas are used for predictions - IGNORE THE FINAL TEST RESULTS FROM THIS SCRIPT
            Use the other script to perform train, val, and testing based on different data - once you have determined the best number of epochs for a given hyperparameter set
            """
    ...
    train_val_hyper_parameters_accuracy_path = train_val_hyper_parameters_path + f'train_val_hyper_parameters_{dropout}_{activation}_{loss}_{optimizer}_{epoch}.png'
    ...
    with open(f'{results_directory}/k_fold_cv_overlapping_data_results_related_work_maia_stiber_arch.txt', 'a') as results_file:
        ...
        results_file.write(f'Train, Val plots saved for hyper_parameters_{dropout}_{activation}_{loss}_{optimizer}_{epoch}.png' + '\n')
        results_file.write("------------ train_val_hyper_parameters_ PLOTS ------------" + "\n")
    """
    End: Logging 
    ------------------------------------------------------------------------------------
